...
/**
 * Draws and drives the soccer ball on the preorder page.
 *
...
export function mountBall(root: HTMLElement): () => void {
  ...
  function on(
    ...
    /* ---------------- form ---------------- */
    var form = root.querySelector<HTMLFormElement>("#ftp-form")!;
    on(form, "submit", function (ev) {
      ev.preventDefault();
      var name = root.querySelector<HTMLInputElement>("#ftp-pname")!;
      var mail = root.querySelector<HTMLInputElement>("#ftp-pmail")!;
      if (!name.value.trim()) { name.focus(); return; }
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(mail.value)) { mail.focus(); return; }

      var consent = root.querySelector<HTMLInputElement>("#ftp-pconsent")!;
      var consentRow = root.querySelector<HTMLElement>("#ftp-consentRow")!;
      if (!consent.checked) {
      ...
      var tier = form.querySelector<HTMLInputElement>('input[name="tier"]:checked')!.value;
      var first = name.value.trim().split(/\s+/)[0];
      root.querySelector("#ftp-doneMsg")!.textContent =
        first + ", your " + tier + " rate is held. We'll email you the week FirstTouch opens.";
      form.classList.add("is-done");
      form.querySelector(".done")!.scrollIntoView({ block: "center", behavior: reduced ? "auto" : "smooth" });
    });
