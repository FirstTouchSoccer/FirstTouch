...
export function AnalysisChat({
  ...
  const atCap = questionsUsed >= MAX_CHAT_TURNS
  ...
  async function send() {
    ...
    try {
      ...
      replyText = res.ok ? data.reply : buildMockChatReply(clip.feedback, clip.metrics, question)
    } catch {
      replyText = buildMockChatReply(clip.feedback, clip.metrics, question)
    }
    ...
    <div className="mt-4 rounded-2xl border border-border bg-card p-4">
      ...
      {atCap ? (
        <p className="mt-3 text-[11px] text-muted-foreground">
          You&apos;ve used all {MAX_CHAT_TURNS} questions for this clip. Upload a new one to keep the
          conversation going.
        </p>
      ) : (
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            placeholder="e.g. Why is my balance score lower?"
            disabled={sending}
      ...
      {!atCap && (
        <p className="mt-2 text-[10px] text-muted-foreground">
          {questionsUsed} of {MAX_CHAT_TURNS} questions used
        </p>
      )}
    </div>
  )
